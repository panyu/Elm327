...
namespace Elm327.Driver
{
    ...
    public class ElmDriver : DataRecievedBase
    {
            ...
            set {
                if (!string.IsNullOrWhiteSpace(value))
                {
                    ...
                    if (processing.Count > 0 && value.Length >= (processing.Peek() + " \r\n").Length)
                    {
                        ...
                        client.Emit("response", new { Command = processing.Count == 0 ? "output" : processing.Dequeue(), Output = value });
                    }
        ...
        Dictionary<string, OBDPid> pidMap = new Dictionary<string, OBDPid>();

        Timer timer = new Timer(50);
        Client client = new Client("http://localhost:8080/server.js");
        ...
        public ElmDriver(string portName, int baudRate)
        {
            ...
            client.HeartBeatTimerEvent += client_HeartBeatTimerEvent;
            client.Error += client_Error;
            client.ConnectionRetryAttempt += client_ConnectionRetryAttempt;
            buildPidMap();
            client.Connect();
        }

        private void buildPidMap()
        {
            var pids = Enum.GetValues(typeof(OBDPid)).Cast<OBDPid>();
            foreach (var pid in pids)
                if(!pidMap.ContainsKey(pid.GetStringValue()))
                    pidMap.Add(pid.GetStringValue(), pid);
        }

        void client_ConnectionRetryAttempt(object sender, EventArgs e)
        {
            Debug.WriteLine("websocket connection retry");
        }
        ...
        void client_HeartBeatTimerEvent(object sender, EventArgs e)
        {
                ...
                if (requests.Count > 0)
                {
                    if (requests.Count > MAX_SIMULTANEOUS_COMMANDS)
                    {
                        for (int i = 0; i < MAX_SIMULTANEOUS_COMMANDS; i++)
                        {
                            ...
                            var pids = request.Split(new char[] { ' ' }, StringSplitOptions.RemoveEmptyEntries);
                            foreach (var pid in pids)
                            {
